...
__all__ = ["SealsWithClubsHandHistory", "Notes"]
...
class SealsWithClubsHandHistory(hh._SplittableHandHistoryMixin, hh._BaseHandHistory):
    """Parses Seals With Clubs Cash or Tournament hands."""
    ...
    _header_re = re.compile(
        r"""
                        ...
                        \)\s+
                        -\s+(?P<date>.+)                              # date in UTC
                        """,
        re.VERBOSE,
    )
    _table_re = re.compile(
        r"^Table '(?P<table_name>.*)'\((?P<table_id>\d+)\)\s+(?P<max_players>\d+)-max .+ Seat #(?P<button>\d+) is the button"
    )
    _seat_re = re.compile(
        r"^Seat (?P<seat>\d+):\s+(?P<name>.+?)\s+\((?P<stack>[\d, ,]+(\.\d+)?) in chips\)"
    )  # noqa
    ...
    def _parse_table(self):
        ...
        self.table_name = self._table_match.group('table_name')
        self.table_id = self._table_match.group('table_id')
        self.max_players = int(self._table_match.group('max_players'))
    ...
    def _parse_players(self):
        ...
        for line in self._splitted[2:]:
            ...
            self.players[index] = hh._Player(
                ...
                stack=float("".join(match.group("stack").split(","))),
